...
exports.toRequiredSchema = toRequiredSchema;
...
function deepSet(path, val, obj) {
  if (!Array.isArray(path) || path.length === 0 || !path[0]) {
    return val;
  }

  if (path.length === 1) {
    return Object.assign({}, obj, _defineProperty({}, path, val));
  }
  ...
  return Object.assign({}, obj, _defineProperty({}, nextProp, deepSet(rest, val, obj ? obj[nextProp] : {})));
}
...
function deepGet(path, obj) {
  ...
  if (path.length === 1 && (typeof obj === "undefined" ? "undefined" : _typeof(obj)) === "object") {
    return obj[path];
  }
  ...
  if (_typeof(obj[nextProp]) === "object") {
...
function toRequiredSchema(rawSchema) {
  var requiredSchema = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : { $required: false };
  var uiSchema = arguments[2];
  var definitions = arguments[3];
  var formData = arguments[4];
  var formContext = arguments[5];

  var schema = retrieveSchema(rawSchema, definitions);

  if (schema.type === "object") {
    return Object.keys(schema.properties).reduce(function (newSchema, nextProp) {
      var inRequiredArray = schema.required && schema.required.some(function (f) {
        return f === nextProp;
      });

      var schemaToUpdate = newSchema;
      if (inRequiredArray && !newSchema[nextProp]) {
        schemaToUpdate = Object.assign({}, schemaToUpdate, _defineProperty({}, nextProp, { $required: true }));
      }

      var nextSchema = toRequiredSchema(schema.properties[nextProp], schemaToUpdate[nextProp], uiSchema ? uiSchema[nextProp] : undefined, definitions, formData, formContext);

      if (nextSchema === schemaToUpdate[nextProp]) {
        return schemaToUpdate;
      } else {
        return Object.assign({}, schemaToUpdate, _defineProperty({}, nextProp, nextSchema));
      }
    }, requiredSchema || { $required: false });
  } else if (schema.type === "array") {
    return toRequiredSchema(schema.items, requiredSchema, uiSchema ? uiSchema.items : undefined, definitions, formData, formContext);
  }

  if (uiSchema) {
    var required = uiSchema["ui:required"];
    if (typeof required === "function") {
      var isRequired = required(formData, formContext);
